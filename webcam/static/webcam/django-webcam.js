function SingleMonitor(options) {
  ...
  options.format = (typeof options.format == 'undefined') ? 'png' : options.format;
  ...
  this.show_monitor = function (format, widget, monitor) {
    ...
    $('#snap').unbind('click').click(function (event) {
  ...
  window.cameramonitor.show_monitor('png', $('#data_id_photo'), $('#id_photo-image'));
}
